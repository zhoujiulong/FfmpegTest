...
void startPlay(JNIEnv *env, jstring videoPath, jobject surface) {
    ...
    if (open_state < 0) {
        ...
        if (av_strerror(open_state, errbuf, sizeof(errbuf)) == 0) {
            FFLOGE("打开视频输入流信息失败，失败原因： %s", errbuf);
        }
        return;
    }
    ...
    FFLOGE("当前视频数据，包含的数据流数量：%d", formatContext->nb_streams);
    ...
    //循环读取数据流的下一帧
    while (av_read_frame(formatContext, packet) == 0 && !stopPlay) {
        ...
        if (packet->stream_index == video_stream_index) {
            ...
            if (sendPacketState == 0) {
                ...
                if (receiveFrameState == 0) {
                    ...
                    FFLOGE("从解码器-接收-数据失败：AVERROR(EAGAIN)");
                } else if (receiveFrameState == AVERROR_EOF) {
                    FFLOGE("从解码器-接收-数据失败：AVERROR_EOF");
                } else if (receiveFrameState == AVERROR(EINVAL)) {
                    FFLOGE("从解码器-接收-数据失败：AVERROR(EINVAL)");
                } else {
                    FFLOGE("从解码器-接收-数据失败：未知");
                }
            } else if (sendPacketState == AVERROR(EAGAIN)) {//发送数据被拒绝，必须尝试先读取数据
                FFLOGE("向解码器-发送-数据包失败：AVERROR(EAGAIN)");//解码器已经刷新数据但是没有新的数据包能发送给解码器
            } else if (sendPacketState == AVERROR_EOF) {
                FFLOGE("向解码器-发送-数据失败：AVERROR_EOF");
            } else if (sendPacketState == AVERROR(EINVAL)) {//遍解码器没有打开，或者当前是编码器，也或者需要刷新数据
                FFLOGE("向解码器-发送-数据失败：AVERROR(EINVAL)");
            } else if (sendPacketState == AVERROR(ENOMEM)) {//数据包无法压如解码器队列，也可能是解码器解码错误
                FFLOGE("向解码器-发送-数据失败：AVERROR(ENOMEM)");
            } else {
                FFLOGE("向解码器-发送-数据失败：未知");
            }
        }
...
Java_com_zhoujiulong_ffmpegtest_FfmpegNative_play(JNIEnv *env, jobject instance,
                                                  jstring videoPath, jobject surface) {
    if (playing) {
        stopPlay = true;
    } else {
        startPlay(env, videoPath, surface);
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_zhoujiulong_ffmpegtest_FfmpegNative_stopPlay(
